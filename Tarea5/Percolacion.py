...
class Grafo:
    ...
    def conexionesAleatorias(self,prob):
        ...
        for n in self.nodos:
            if n is not self.nodos[len(self.nodos) - 1]:
                for m in self.nodos:
    ...
    def ford_fulkersonN(self):
         with open ("FordFulkersonN.csv", "at") as crear:
             self.s = self.nodos[0]
    ...
    def ford_fulkersonA(self):
         with open ("FordFulkersonA.csv", "at") as crear:
             self.s = self.nodos[0]
             self.t = self.nodos[len(self.nodos) - 1]
             if self.s == self.t:
                 return 0
    ...
    def percolaNodos(self):
        self.auxIndice = []
        for pn in range(k**2-2*k+2):
            self.auxNodo = []
            self.nuevosNodos()
            self.ford_fulkersonN()
    ...
    def percolaAristas(self):
        ...
        for pn in range(1,11):
            self.NuevasAristas()
            self.ford_fulkersonA()


    def graficar(self):
        with open("Tarea5Completo.plot","w") as archivo:
             print("set term png", file=archivo)
             print("set output 'Tarea5Completo.png", file=archivo)
...
for umbral in range(1,5):
     ...
     for rep in range(0,10):
          ...
          if percolaNodo == 1:
               with open ("TiempoPercolaNodo.csv", "at") as percola:
          ...
          if percolaArista == 1:
               with open ("TiempoPercolaArista.csv", "at") as percola:
                    TiempoInicio = time.clock()
                    G = Grafo(k,prob)
                    G.agrega(k)
                    G.conexiones(umbral)
