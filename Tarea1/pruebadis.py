...
t=1
with open ("nodosCiclo.dat","w") as crear:
    for t in range(n):
        x=random()
        y=random()
        nodos.append((x,y))
...
with open("tarea1Ciclo.plot","w") as archivo:
    print("set term png", file=archivo)
    print("set output 'graficaCiclo.png'", file=archivo)
    ...
    print("plot 'nodosCiclo.dat' using 1:2:3 with points pt 7 lc var", file=archivo)
