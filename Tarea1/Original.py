...
nodos=[]
aristas=[]
with open ("OriginalNodos.dat", "w") as crear:
    aux=1
    for t in range(n):
        x=random()
        y=random()
...
with open("Original.plot","w") as archivo:
    print("set term png", file=archivo)
    print("set output 'OriginalGraf.png'", file=archivo)
    ...
    print("plot 'OriginalNodos.dat' using 1:2 with points pt 7", file=archivo)
