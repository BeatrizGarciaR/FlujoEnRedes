...
class Grafo:
    ...
    def conecta(self, k):
        for r in range(1,k+1):
            for j in range(0,i):
                a = self.nodos[j]
                b = self.nodos[j-r]
                self.aristas[(a,b)] = self.aristas[(b,a)] = distancia(a,b)
                ...
                if rand < p:
                    if ((x1,y1),(x2,y2)) not in self.aristas:
                        if ((x2,y2),(x1,y1)) not in self.aristas:
                            ...
                            if u is not v:
                                dis = abs(u-v)
                                self.aristas[((x1,y1),(x2,y2))] = self.aristas[((x2,y2),(x1,y1))] = distancia((x2,y2),(x1,y1))
    ...
    def DistanciaPromedio(self):
        self.s = floor((3*Tk)-(i/4))
        with open ("DistanciaPromedio.csv", "at") as distancia:
            self.sumDis = 0
            for u in self.d:
    ...
    def DensidadPromedio(self):
        with open ("DensidadPromedio.csv", "at") as crear:
            self.densidad = []
            for (x,y) in self.nodos:
                self.lpq = []
...
Tk = (i/4)
TK = floor(i/4)
probabilidad = [.05, .1, .15, .2, .25, .3, .35, .4, .45, .5]
for prob in probabilidad:
    print(prob)
    for vark in range(1,TK+1):
        k = vark
        print(k)
        p = prob
        G = Grafo()
